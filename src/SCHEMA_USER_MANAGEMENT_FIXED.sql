-- ============================================
-- USER MANAGEMENT SCHEMA FOR BTCWHEEL - FIXED
-- ============================================
-- Esegui questo file nel SQL Editor di Supabase
-- Dashboard → SQL Editor → New Query → Incolla e Run
-- ============================================

-- 1️⃣ USER PROFILES (estensione dati utente)
...
-- 2️⃣ SUBSCRIPTION PLANS (catalogo piani disponibili)
...
-- 3️⃣ USER SUBSCRIPTIONS (piano attivo per ogni utente)
CREATE TABLE IF NOT EXISTS user_subscriptions (
  ...
  -- Payment tracking (opzionale, per integrazioni future)
  ...
  -- Un utente può avere solo una subscription attiva alla volta
  UNIQUE(user_id)
);
...
-- 4️⃣ USER BACKUPS (storico backup per disaster recovery)
CREATE TABLE IF NOT EXISTS user_backups (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  
  -- Backup data (full snapshot JSON)
...
-- 5️⃣ ADMIN AUDIT LOG (tracking modifiche admin)
...
VALUES 
  ...
  (
    ...
    '{
      "max_strategies": 999,
      "max_active_trades": 999,
      "ai_chat_messages_per_day": 999,
      "historical_data_days": 999,
      "priority_support": true,
      "advanced_analytics": true,
      "custom_indicators": true,
      "community_access": true,
...
-- USER BACKUPS: Users can read their own backups
...
-- ============================================
-- VIEWS: Useful admin queries
-- ============================================

-- Drop existing view
DROP VIEW IF EXISTS admin_users_overview;

-- View: Users with their current subscription
-- SAFE VERSION: Checks if wheel tables exist before querying
...
SELECT 
  ...
  0 as total_strategies,  -- Placeholder - update manually if wheel_strategies exists
  0 as total_trades       -- Placeholder - update manually if wheel_trades exists
FROM auth.users u
...
-- DONE! ✅
-- ============================================
-- Se hai le tabelle wheel_strategies e wheel_trades, esegui questo per aggiornare la VIEW:
--
-- CREATE OR REPLACE VIEW admin_users_overview AS
-- SELECT 
--   u.id,
...
--   us.status as subscription_status,
--   us.expires_at as subscription_expires,
--   (SELECT COUNT(*) FROM wheel_strategies ws WHERE ws.user_id = u.id) as total_strategies,
--   (SELECT COUNT(*) FROM wheel_trades wt WHERE wt.user_id = u.id) as total_trades
-- FROM auth.users u
-- LEFT JOIN user_profiles up ON u.id = up.user_id
-- LEFT JOIN user_subscriptions us ON u.id = us.user_id
-- LEFT JOIN subscription_plans sp ON us.plan_id = sp.id
