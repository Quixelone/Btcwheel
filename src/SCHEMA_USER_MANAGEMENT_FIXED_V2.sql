-- ============================================
-- USER MANAGEMENT SCHEMA FOR BTCWHEEL - FIXED V2
-- ============================================
...
-- View: Users with their current subscription (SAFE VERSION)
-- NON fa riferimento a tabelle wheel_strategies/wheel_trades
...
-- 2. Testa l'auto-creazione del profilo registrando un nuovo utente
-- 3. Se hai le tabelle wheel_strategies e wheel_trades con colonna user_id,
--    puoi aggiornare manualmente la view con:
--
-- CREATE OR REPLACE VIEW admin_users_overview AS
-- SELECT 
--   u.id,
...
--   us.status as subscription_status,
--   us.expires_at as subscription_expires,
--   COALESCE((SELECT COUNT(*) FROM wheel_strategies ws WHERE ws.user_id = u.id), 0) as total_strategies,
--   COALESCE((SELECT COUNT(*) FROM wheel_trades wt WHERE wt.user_id = u.id), 0) as total_trades
-- FROM auth.users u
-- LEFT JOIN user_profiles up ON u.id = up.user_id
-- LEFT JOIN user_subscriptions us ON u.id = us.user_id
-- LEFT JOIN subscription_plans sp ON us.plan_id = sp.id
